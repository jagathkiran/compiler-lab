...
void findFirst(char symbol, char *res) {
    ...
    for (int i=0;i<n;i++) {
        if (prod[i][0] == symbol) {
            ...
            if (rhs[0] == EPSILON) add(res, EPSILON);
            else {
                for (int j=0; rhs[j]; j++) {
                    char temp[MAX] = "";
                    findFirst(rhs[j], temp);
                    for (int k=0; temp[k]; k++)
                        if (temp[k] != EPSILON) add(res, temp[k]);
                    if (!strchr(temp, EPSILON)) break; // Stop if epsilon not present
                    if (!rhs[j+1]) add(res, EPSILON);   // All symbols can be epsilon
                }
            }
        }
    }
    ...
    do {
        ...
        for (int i=0;i<n;i++) {
            ...
            for (int j=0; rhs[j]; j++) {
                if (isupper(rhs[j])) {
                    int idx=-1;
                    for (int t=0;t<ntCount;t++) if(nonTerminals[t]==rhs[j]) idx=t;
                    ...
                    while (rhs[k]) {
                        if (isupper(rhs[k])) {
                            ...
                            for (int t=0; temp[t]; t++) {
                                if (temp[t] != EPSILON) add(follow[idx], temp[t]);
                                else epsilon = 1;
                            }
                            if (!epsilon) break;
                        } else {
                        ...
                        k++;
                        if (!rhs[k]) addFollow = 1; // If end reached
                    }

                    if (!rhs[j+1] || addFollow) {
                        int lhsIdx=-1;
                        for (int t=0;t<ntCount;t++) if(nonTerminals[t]==lhs) lhsIdx=t;
                        for (int t=0; follow[lhsIdx][t]; t++) add(follow[idx], follow[lhsIdx][t]);
                    }

                    if (strlen(follow[idx]) > oldLen) changed = 1;
                }
            }
        }
    } while(changed);
...
int main() {
    ...
    findFollowSets();
    printf("\nFOLLOW sets:\n");
    for (int i=0;i<ntCount;i++) printf("FOLLOW(%c) = { %s }\n", nonTerminals[i], follow[i]);
